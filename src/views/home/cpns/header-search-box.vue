<template>
  <div class="header-search-box">
    ...
    <!-- 日期组件 -->
    <DateRangePicker v-model:show="show" v-model="range" @confirm="onConfirm" />
    ...
    <div class="hotSuggests section">
      <template v-for="(item, index) in props.hotSuggests" :key="index">
        <div
          class="hotSuggest"
          :style="{
...
const range = computed({
  get: (): [string, string] => [mainStore.startDate, mainStore.endDate] as [string, string],
  set: (val: [string, string]) => {
    mainStore.startDate = val[0]
    mainStore.endDate = val[1]

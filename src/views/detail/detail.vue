<template>
  <div ref="detailRef" class="detail">
    <tab-control
      ...
      class="tab-control"
      :title="names"
      @item-click="outItemClick"
    />
    <div v-if="!showReturnTop" class="return-top" @click="returnTop">
      <Icon name="back-top" class="h-8 w-8 text-[var(--primary-color)]" />
    </div>
    <Navbar title="房屋详情" left-text="旅途" @click-left="onClickLeft" />
    <div v-if="mainPart" v-memo="[mainPart]" class="mainPart">
      <Swiper v-if="housePics?.length" :house-pics="housePics" />
      <Infos :ref="getSectionRefs" name="简介" :house-info="mainPart.topModule" />
      <Facility
        :ref="getSectionRefs"
        name="设施"
      ...
      />
      <Map :ref="getSectionRefs" name="位置" :position="mainPart.dynamicModule.positionModule" />
      <Notice
        :ref="getSectionRefs"
        name="规则"
        :notices="mainPart.dynamicModule.rulesModule.orderRules"
      />
      <PriceIntro :ref="getSectionRefs" name="价格" :introduction="mainPart.introductionModule" />
    </div>
  </div>
</template>
